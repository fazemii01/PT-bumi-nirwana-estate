...
import { BuildingFloorPlans, BuildingImage, BuildingKprRules } from "@/types/building-properties";
...
type MediaEdit = {
  ...
  setImagesMeta: React.Dispatch<React.SetStateAction<{ caption?: string; sort_order?: number }[]>>;
  newFloorFiles: File[];
  setNewFloorFiles: React.Dispatch<React.SetStateAction<File[]>>;
  floorPlansMeta: { name?: string }[];
  setFloorPlansMeta: React.Dispatch<React.SetStateAction<{ name?: string; sort_order?: number }[]>>;
  newKprFiles: File[];
  setNewKprFiles: React.Dispatch<React.SetStateAction<File[]>>;
  kprRulesMeta: BuildingKprRules[];
  setKprRulesMeta: React.Dispatch<React.SetStateAction<BuildingKprRules[]>>;
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                          ...
                          <div key={img.id ?? i} className="border rounded-lg p-2 bg-gray-50 space-y-2">
                            <div className="w-full aspect-square bg-white border rounded flex items-center justify-center overflow-hidden">
                              {img.image_url ? (
                                ...
                                <img
                                  src={imgUrl(`building_property/building_images/${img.image_url}`)}
                                  alt={`image-${i}`}
                                  className="w-full h-full object-cover cursor-pointer"
                                  onClick={() => openPreview(imgUrl(`building_property/building_images/${img.image_url}`))}
                                />
                              ) : (
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  <h4 className="font-medium text-sm text-gray-700">Denah lama ({originalFloorPlans!.length})</h4>
                            ...
                            <div className="w-full aspect-square bg-white border rounded flex items-center justify-center overflow-hidden">
                              {fp.file_url ? (
                                isImage ? (
                                  // eslint-disable-next-line @next/next/no-img-element
                                  <img
                                    src={imgUrl(`building_property/building_floor_plans/${fp.file_url}`)}
                                    alt={`fp-${i}`}
                                    className="w-full h-full object-cover cursor-pointer"
                                    onClick={() => openPreview(imgUrl(`building_property/building_floor_plans/${fp.file_url}`))}
                                  />
                                ) : (
                ...
                <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center h-[200px] flex flex-col items-center justify-center">
                  <Upload className="w-8 h-8 mx-auto mb-2 text-gray-400" />
                  <p className="text-sm text-gray-600 mb-2">Upload denah lantai satu per satu atau multi-select</p>
                  <input
                    type="file"
                    multiple
                    accept="image/*"
                    onChange={(e) => {
                      ...
                      setFloorPlansMeta((prev) => [...prev, ...files.map(() => ({ name: "" }))]);

                      e.currentTarget.value = "";
                    }}
                    className="hidden"
                    id="floorPlans-upload-edit"
                  />
                  <Button type="button" variant="outline" className="cursor-pointer" onClick={() => document.getElementById("floorPlans-upload-edit")?.click()}>
                    <Plus className="w-4 h-4 mr-2" />
                    Tambah Denah
                  </Button>
                </div>
                ...
                {newFloorFiles.length > 0 && (
                  <div className="space-y-4">
                    <h4 className="font-medium text-sm text-gray-700">Denah baru ({newFloorFiles.length})</h4>
                    <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2">
                            ...
                            <div className="flex-shrink-0 relative">
                              ...
                              <div className="absolute -top-2 -right-2">
                                <Button
                                  ...
                                  className="h-6 w-6 p-0 rounded-full cursor-pointer"
                                  onClick={() => {
                                    setNewFloorFiles((prev) => prev.filter((_, i) => i !== index));
                                    setFloorPlansMeta((prev) => prev.filter((_, i) => i !== index));
                                  }}
                                >
                                  <Trash2 className="w-3 h-3" />
                                </Button>
            ...
            <Card>
              <CardHeader>
                <CardTitle>Aturan Kpr</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                {/* Existing floor plans */}
                <div className="space-y-2">
                  <h4 className="font-medium text-sm text-gray-700">File aturan kpr lama ({originalKprRules!.length})</h4>
                  {originalKprRules!.length ? (
                            ...
                            <div className="w-full aspect-square bg-white border rounded flex items-center justify-center overflow-hidden">
                              {fp.file_url ? (
                                isImage ? (
                                  // eslint-disable-next-line @next/next/no-img-element
                                  <img
                                    src={imgUrl(`building_property/building_kpr_rules/${fp.file_url}`)}
                                    alt={`fp-${i}`}
                                    className="w-full h-full object-cover cursor-pointer"
                                    onClick={() => openPreview(imgUrl(`building_property/building_kpr_rules/${fp.file_url}`))}
                                  />
                                ) : (
                                  <div className="text-xs text-gray-500 text-center">
                                    <div>PDF</div>
                    ...
                    <div className="text-sm text-muted-foreground">Tidak ada aturan kpr lama.</div>
                  )}
                </div>
                ...
                <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center h-[200px] flex flex-col items-center justify-center">
                  <Upload className="w-8 h-8 mx-auto mb-2 text-gray-400" />
                  <p className="text-sm text-gray-600 mb-2">Upload file aturan kpr</p>
                  <input
                    type="file"
                    multiple
                    ...
                    onChange={(e) => {
                      ...
                      setKprRulesMeta((prev) => [...prev, ...files.map((): BuildingKprRules => ({ file_url: "" }))]);
                      e.currentTarget.value = "";
                    }}
                    className="hidden"
                    id="kprRules-upload-edit"
                  />
                  <Button type="button" variant="outline" className="cursor-pointer" onClick={() => document.getElementById("kprRules-upload-edit")?.click()}>
                    <Plus className="w-4 h-4 mr-2" />
                    Tambah Denah
                  </Button>
                </div>
                ...
                {newKprFiles.length > 0 && (
                  <div className="space-y-4">
                    <h4 className="font-medium text-sm text-gray-700">File aturan baru ({newKprFiles.length})</h4>
                    <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2">
                            ...
                            <div className="flex-shrink-0 relative">
                              ...
                              <div className="absolute -top-2 -right-2">
                                <Button
                                  ...
                                  className="h-6 w-6 p-0 rounded-full cursor-pointer"
                                  onClick={() => {
                                    setNewKprFiles((prev) => prev.filter((_, i) => i !== index));
                                    setKprRulesMeta((prev) => prev.filter((_, i) => i !== index));
                                  }}
                                >
                                  <Trash2 className="w-3 h-3" />
                                </Button>
