...
import { BuildingProperty, BuildingStatus, Specifications } from "@/types/building-properties";
...
const PropertyBuilding = ({ buildings, nameProperty }: { buildings: BuildingProperty[]; nameProperty: string }) => {
  ...
  return (
    <TabsContent value="buildings">
                    ...
                    <CardContent className="space-y-4">
                      <div className="grid grid-cols-2 gap-3 text-sm">
                        ...
                        <div>
                          ...
                          <p className="font-medium">{building.price != null ? formatCurrency(building.price) : "_"}</p>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-2">
                        <Link href={`/properties/building-properties/edit/${building.id}`}>
                          <Button variant="outline" size="lg" className="cursor-pointer flex-1 w-full h-9 rounded-lg flex items-center justify-center border bg-gray-200  text-sm font-medium  hover:border-gray-300 transition-colors">
                            <Edit className="w-4 h-4 sm:mr-2" />
                            <span className="hidden sm:inline">Edit</span>
                          </Button>
                        </Link>

                        <Button variant="outline" size="sm" className="cursor-pointer flex-1 w-full h-9 rounded-lg flex items-center justify-center border  bg-gray-200 text-sm font-medium  hover:border-gray-300 transition-colors">
      ...
      <ConfirmMessage open={open} setOpen={setOpen} data={selectedId ?? ""} onConfirm={handleDelete} isLoading={isLoading} />
    </TabsContent>
  );
};
